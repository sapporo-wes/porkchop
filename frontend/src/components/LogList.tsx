...
import { ValidationLog, ValidationLogDetail } from '../types';
...
const LogList: React.FC<LogListProps> = ({ onError }) => {
  ...
  const { data: logsData, isLoading: logsLoading, error: logsError } = useQuery({
    ...
    queryFn: () => apiService.getValidationLogs(currentPage, pageSize, searchTerm),
  });

  const { data: logDetail, isLoading: detailLoading } = useQuery({
    queryKey: ['log-detail', selectedLogId],
    queryFn: () => selectedLogId ? apiService.getValidationLogDetail(selectedLogId) : null,
                    ...
                    <div className="flex items-center space-x-4">
                      {log.average_score && (
                        <div className="text-right">
                          ...
                          <p className={`text-lg font-bold ${getScoreColor(log.average_score)}`}>
                            {log.average_score}
                          </p>
                        </div>
                      )}
              ...
              {detailLoading ? (
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <h4 className="font-medium text-gray-900 mb-2">バッチ情報</h4>
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        <span className="text-gray-600">バッチID:</span>
                        <span className="ml-2 font-mono text-xs">{logDetail.batch_id}</span>
                      </div>
                      <div>
                        <span className="text-gray-600">作成日時:</span>
                        <span className="ml-2">{formatDate(logDetail.created_at)}</span>
                      </div>
                      <div>
                        <span className="text-gray-600">ステータス:</span>
                        <span className={`ml-2 px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(logDetail.status)}`}>
                          {logDetail.status === 'completed' ? '完了' : 
                           logDetail.status === 'processing' ? '処理中' : '失敗'}
                        </span>
                      </div>
                      <div>
                      ...
                      <div key={file.file_id} className="border border-gray-200 rounded-lg p-4">
                        ...
                        {file.validation_result && (
                          <div className="space-y-3">
                            {/* 問題一覧 */}
                            {file.validation_result.issues && file.validation_result.issues.length > 0 && (
                              <div>
                                <h6 className="text-sm font-medium text-gray-700 mb-2">検出された問題:</h6>
                                <div className="space-y-2">
                                  {file.validation_result.issues.map((issue, index) => (
                            ...
                            {/* 推奨事項 */}
                            {file.validation_result.recommendations && file.validation_result.recommendations.length > 0 && (
                              <div>
                                <h6 className="text-sm font-medium text-gray-700 mb-2">推奨事項:</h6>
                                <ul className="text-sm text-gray-700 space-y-1">
                                  {file.validation_result.recommendations.map((rec, index) => (
